...
- liquidctl, GPL-3.0 (https://github.com/liquidctl/liquidctl): the read-only NZXT Kraken X3-family telemetry reader (`src/PCHelper.Adapters/KrakenX3TelemetryReader.cs`) derives its device IDs, status-report marker, and field offsets from `liquidctl/driver/kraken3.py` (reviewed 2026-07-15). No liquidctl source code is copied; the derivation is licence-compatible with this repository's GPL-3.0. RigPilot's reader performs no HID writes — the firmware streams status reports unsolicited — and implements no pump, fan, or lighting control from this protocol.
